...
| `FolioSquare.vue` | **穹庭书局 · 星笺广场主页** (`/folios`)。银河渐变背景+固定顶栏（返航/品牌/搜索/写星笺/刷新/回天际）+ 筛选条（分类 tab / 排序 chip / 搜索）+ 官方「星河八卷轴」横向画卷（仅 visibility=galaxy 官方星笺）+ 本周推荐三笺（官方 3 本 + 热榜补足，用 FolioGrid 大卡）+ 书架栅格（FolioGrid，分页无限滚动；showOwner 显示作者名；非 owner 点击打开 `/folios/:id` 全屏页）+ **右下角回顶部悬浮按钮**（顶部栏滚出可视区域时显示，点击平滑回顶；已放弃"下滑隐藏/上滑弹出"方案）。**搜索下拉**：防抖 250ms 拉取 `/api/collections/public?search=`，最多展示 20 条（limit=20），面板限高 320px 带自定义金色滚动条 |
