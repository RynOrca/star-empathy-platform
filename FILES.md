...
## showcase/ — 复赛作品演示页面（纯静态 · 无需后端）

> 不需要录制视频：打开后自动播放，6 幕约 90 秒循环；键盘 ← → Space Esc，右指示点可直接跳转。
> 设计针对 2560×1600 大屏做响应式，小屏允许滚动查看完整内容。

| 文件 | 用途 |
|---|---|
| `index.html` | **演示入口**。顶部控制栏、舞台 stage、底部字幕旁白栏三大部分；引用 styles.css、scenes.js、player.js |
| `styles.css` | **全套样式**。深空金紫配色 token 对齐主项目；6 幕场景分版样式 + 响应式；全局自定义滚动条覆盖浏览器默认样式；v3（03左栏占满/04书局清爽/灵动动画）和 v4（AI 分析页 2560×1600 适配）两个升级区块；小高度屏幕滚动兜底 |
| `scenes.js` | **6 幕场景定义**。SCENES 数组导出到 window：①封面双内核、②真实天球、③天镜览星、④心事归星、⑤穹庭书局、⑥AI聚合读星；每幕 render(el) 输出 DOM 并附带动画（打字机、标签切换、手风琴、候选卡选中高亮）；重点强调真星坐标（非虚构）和真实故事（非纯天文） |
| `player.js` | **自动播放引擎**（IIFE）。goTo 场景切换、打字机旁白字幕、帧驱动的时长进度条；键盘 Space/←/→/Esc；右指示点点击跳转；背景粒子画布。暴露 SCENES 供调试 |
| `README.txt` | 打开方式说明 + 6 幕流程 + 操作快捷键 |
